...
export const Form = () => {
  ...
  return (
    <div className="form">
      <div className="title35 green">{__('client.form_header',sharedData)}</div>
      {inputs.map((item, index) => {
        return (
          <input key={index} type={item.type} id={item.id} placeholder={item.placeholder} onChange={handleChange} />
        );
      })}
